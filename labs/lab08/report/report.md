...
![Удаление файла](image/lab8.10.png){#fig:010 width=70%}
  ...
  Запускаю в консоли в фоновом режиме (с помощью символа &) редактор mousepad, потому что редактора gedit у меня, к сожалению нет, но работают они идентично (рис. [-@fig:011]).

![Создание фонового процесса](image/lab8.11.png){#fig:011 width=70%}
  
  С помощью утилиты ps определяю идентификатор процесса mousepad, его значение 60765 (рис. [-@fig:012]). Также мы можем определить идентификатор с помощью pgrep.
...
![Чтение документации](image/lab8.18.png){#fig:018 width=70%}
  ...
  Вывела имена всех директорий, имеющихся в моем домашнем каталоге, используя аргумент d у утилиты find опции -type, то есть указываю тип файлов, который мне нужен и этот тип Директория (рис. [-@fig:019]). 
